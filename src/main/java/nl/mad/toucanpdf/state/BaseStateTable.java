...
public class BaseStateTable extends AbstractTable implements StateTable {
    ...
    private void fillEmptyCells(List<TableRow> rows, List<StateCell> tableContent) {
        for (int currentRow = 0; currentRow < rows.size(); ++currentRow) {
            ...
            for (int i = 0; i < this.columnAmount; ++i) {
                if (content[i] == null && columnsNotInUse(i, 1, rows, currentRow)) {
    ...
    private void divideColumnsOverRows(List<StateCell> tableContent, List<TableRow> rows) {
        ...
        currentRow = AddHeader(rows, currentRow);
        ...
        for (StateCell c : tableContent) {
            ...
            while (!validSpaceFound) {
                ...
                if (validColumn && columnsNotInUse(currentColumn, c.getColumnSpan(), rows, currentRow)) {
        ...
        SetNewHeader(tableContent, rows);
    }

    private void SetNewHeader(List<StateCell> tableContent, List<TableRow> rows) {
    ...
    private int AddHeader(List<TableRow> rows, int currentRow) {
    ...
    private double[] calculateColumnWidths(List<TableRow> rows) {
        ...
        //iterate through each cell per row and determine for each column what the largest width is
        ...
        remainingWidth = DivideWidthForMinValues(rows, columnWidths, remainingWidth);
    ...
    private void matchTotalRequiredWidths(double[] columnWidths, double remainingWidth) {
        ...
        if (remainingWidth > 0) {
            SpreadRemainingWidthOverColumns(columnWidths, remainingWidth, nonFulfilledMaxPossibleWidths, smallMaxPossibleWidths);
        }
    }

    private void SpreadRemainingWidthOverColumns(double[] columnWidths, double remainingWidth,
            List<ColumnPossibleWidth> nonFulfilledMaxPossibleWidths, List<ColumnPossibleWidth> smallMaxPossibleWidths) {
    ...
    private double DivideWidthForMinValues(List<TableRow> rows, double[] columnWidths, double remainingWidth) {
        ...
        /**the required cell widths have already been set, so we can filter those out, we'll also want to sort by size to make sure 
           that overly large min. widths aren't processed unless we actually have the required width available*/
        List<ColumnPossibleWidth> requiredSortedMinWidths = allMinPossibleWidths
                .stream()
                .filter(minFilter)
        ...
        //if there is not enough space to meet all min required values, we'll spread the remainder procentually
        if(notEnoughSpace && remainingWidth > 0) {
            List<ColumnPossibleWidth> largeWidths = allMinPossibleWidths
                    .stream()
            ...
            spreadRemainingWidthProcentually(columnWidths, remainingWidth, largeWidths, totalExtraWidthRequired);
            remainingWidth = 0;
        }
    ...
    private void setMaxHeightForRow(double maxHeight, StateCell highest, TableRow row) {
    ...
    private Cell getCellAbove(List<TableRow> rows, int currentRow, int cellCount) {
        Cell c = null;
     ...
     * @return
     */
    private boolean columnsNotInUse(int currentColumn, int columnSpan, List<TableRow> rows, int currentRow) {
