...
public class BaseStateImageTest {
	...
	public void processContentSizeTest(@Mocked final StatePage page) {
		new NonStrictExpectations() {
			{
				...
				returns(new LinkedList<int[]>(Arrays.asList(new int[] {10, 200, 200})));
			}
		};
		image.on(0, 200);
		...
		assertEquals(new Position(50, 200), image.getPosition());
		image.align(Alignment.RIGHT);
		image.on(0, 200);
		image.processContentSize(page);
		assertEquals(new Position(90, 200), image.getPosition());
